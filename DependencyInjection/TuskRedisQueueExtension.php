 ...
 */
class TuskRedisQueueExtension extends Extension
{
    /**
     * {@inheritDoc}
     */
    ...
    {
        $class = $container->getParameter('tusk_redis_queue.connection.class');
        $monitorClass = $container->getParameter('tusk_redis_queue.monitor.class');
        foreach ($connections as $name => $connection) {
            $definition = new Definition($class);
            $definition->addArgument(new Reference($connection));
            $container->setDefinition(
                sprintf('tusk_redis_queue.connection.%s', $name),
                $definition
            );
            $definition = new Definition($monitorClass);
            $definition->addArgument(
                new Reference(sprintf('tusk_redis_queue.connection.%s', $name))
            );
            $container->setDefinition(
                sprintf('tusk_redis_queue.monitor.%s', $name),
                $definition
            );
        }
    }
    ...
    {
        $class = $container->getParameter('tusk_redis_queue.producer.class');
        foreach ($producers as $name => $producer) {
            ...
            $container->setDefinition(
                sprintf('tusk_redis_queue.producer.%s', $name),
                $definition
            );
        }
    }
            ...
            if (count($consumer['callback']) == 1) {
                ...
                throw new \UnexpectedValueException(
                    sprintf(
                        ...
                        sprintf('tusk_redis_queue.consumer.%s', $name)
                    )
                );
            }
            $definition->addArgument(
    ...
    {
        $class = $container->getParameter('tusk_redis_queue.rpc_factory.class');
        foreach ($rpcs as $name => $rpc) {
            ...
            $container->setDefinition(
                sprintf('tusk_redis_queue.rpc.%s', $name),
                $definition
            );
        }
    }
